...
use udp_packet_studio::types::{PayloadType, LogExportFormat, InspectorProtocol, AboutTab, SettingsTab, AppTheme, Collection};
use udp_packet_studio::udp_worker::{UdpWorker, UdpEvent};
use udp_packet_studio::types::LoggerCommand;
use udp_packet_studio::locales::LanguageSetting;

fn make_test_state_raw(worker: UdpWorker, rx_event: Receiver<UdpEvent>, tx_logger: Sender<LoggerCommand>) -> UdpStudioState {
    UdpStudioState {
...
fn test_gui_triggered_communication() {
    ...
    use udp_packet_studio::types::LoggerCommand;
    use udp_packet_studio::udp_worker::UdpCommand;
    ...
    state.composer_ip = partner_addr.ip().to_string();
    state.composer_port = partner_addr.port().to_string();
    state.composer_payload = "Hello GUI World!".to_string();
    state.composer_name = "Test Name".to_string();
    state.sockets = vec![udp_packet_studio::types::ActiveSocketState {
        id: "main".to_string(),
...
fn test_collections_gui_interactions() {
    use std::net::UdpSocket;
    use std::sync::mpsc::channel;
    use udp_packet_studio::types::{LoggerCommand, PacketDefinition};
    use udp_packet_studio::udp_worker::UdpCommand;
    ...
    // Construct the state with test values
    let mut state = make_test_state_raw(worker, rx_event, tx_logger);
    state.collections = vec![test_col];
    state.selected_request_id = Some(test_req_id.clone());
    state.composer_ip = "127.0.0.1".to_string();
    state.composer_port = "9000".to_string();
    state.composer_payload_type = PayloadType::Hex;
    state.composer_payload = "AABBCC".to_string();
    state.composer_name = "Composer Request".to_string();
    state.sockets = vec![udp_packet_studio::types::ActiveSocketState {
        id: "main".to_string(),
        name: "Main Socket".to_string(),
        ip: "127.0.0.1".to_string(),
...
fn test_log_limit_and_truncation_gui() {
    use std::sync::mpsc::channel;
    use udp_packet_studio::types::{LoggerCommand, LogEntry, LogDirection};
